...
var ReactVersion = '19.0.0-experimental-ed15d500-20241110';
  ...
  function resolveToJSON(key, value) {
    ...
    if (typeof value === 'object') {
      ...
      if (proto !== ObjectPrototype && (proto === null || getPrototypeOf(proto) !== null)) {
        if (temporaryReferences === undefined) {
          throw new Error('Only plain objects, and a few built-ins, can be passed to Server Actions. ' + 'Classes or null prototypes are not supported.' + (describeObjectForErrorMessage(parent, key) ));
...
function resolveDebugInfo(response, id, debugInfo) {
  ...
  if (debugInfo.owner === null && response._debugRootOwner != null) {
    // $FlowFixMe
    debugInfo.owner = response._debugRootOwner; // We override the stack if we override the owner since the stack where the root JSX
    // was created on the server isn't very useful but where the request was made is.
    // $FlowFixMe
