...
import type { RenderMessage, RequestHandler, StreamPluginOptions } from '../types.js';
import { DEFAULT_CONFIG } from '../types.js';

const __dirname = dirname(fileURLToPath(import.meta.url));

export function createSsrHandler(options: StreamPluginOptions, server: ViteDevServer, clientComponents: Map<string, string>): RequestHandler {
  ...
  return async function handleSsrRequest(req, res, next) {
    if (!req.url || req.url.startsWith('/@') || (req.url.includes('.') && !req.url.endsWith('.html'))) {
      return next();
    }
    ...
    try {
      ...
      const moduleBaseURL = options.moduleBaseURL ? options.moduleBaseURL : 
        options.moduleBase.startsWith('/') ? options.moduleBase : '/' + options.moduleBase;
