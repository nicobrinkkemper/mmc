...
import type { ModuleLoader, RscStreamOptions, StreamPluginOptions, StreamResult } from "./types.js";
import { DEFAULT_CONFIG } from "./types.js";

export function createHandler(
  pluginOptions: StreamPluginOptions, 
  streamOptions: Pick<RscStreamOptions, 'loader' | 'url' | 'temporaryReferences' | 'logger' | 'cssFiles'> & { moduleGraph: any }
) {
  ...
  return createRscStream(pluginOptions, {
    ...
    moduleBase,
    pagePath: typeof pluginOptions.Page === 'function' ? pluginOptions.Page(streamOptions.url) : pluginOptions.Page,
    propsPath: typeof pluginOptions.props === 'function' ? pluginOptions.props(streamOptions.url) : pluginOptions.props,
    logger: streamOptions.logger ?? createLogger(),
    moduleGraph: streamOptions.moduleGraph
  });
}

async function resolveProps(loader: ModuleLoader, path: string, exportName: string, url: string): Promise<{type: "success", props: any} | {type: "error", error: Error} | {type: "skip"}> {
  ...
  if (isPropsFn) {
    try {
      const propsResult = await propsModule[exportName](url);
  ...
  return { type: "error", error: new Error(`Could not find props export "${exportName}" in "${path}".`) };
}

async function resolvePage(loader: ModuleLoader, path: string, exportName: string): Promise<{type: "success", page: any} | {type: "error", error: Error}> {
  ...
  return { type: "error", error: new Error(`Could not find Page export "${exportName}" in "${path}".`) };
}

export async function createRscStream(options: any, params: RscStreamOptions & { moduleGraph: any }): Promise<StreamResult> {
  const { url, controller, loader, Html, moduleBase, pagePath, propsPath, temporaryReferences, logger, moduleGraph } = params;
  try {
    ...
    const props = await resolveProps(safeLoader, propsPath ?? pagePath, params.propsExportName, url);
    if(props.type === "error") return { type: props.type, error: props.error };
    if(props.type === "skip") return { type: props.type };
    const Page = await resolvePage(safeLoader, pagePath, params.pageExportName);
