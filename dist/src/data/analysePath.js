...
export const analyzePath = (theme, path) => {
    ...
    if (segments.length === 0)
        return { ...baseResult, isHome: true, to: "/" };
    // First segment must be a theme
    const pathTheme = segments[0];
    ...
    if (theme && theme !== pathTheme) {
        throw new Error(`Theme mismatch: expected ${theme}, got ${pathTheme} in path`);
    }
    theme = pathTheme;
    switch (segments[1]) {
        case "levels":
...
//# sourceMappingURL=analysePath.js.map