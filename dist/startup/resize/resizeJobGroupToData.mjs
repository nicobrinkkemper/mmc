...
// index: number,
    ...
    const { userInfo: { main }, output: { reference, href, version }, resized: { width, height, aspectRatio, placeholder }, } = job;
    ...
    const srcSet = (!prev?.srcSet ? "" : prev.srcSet + ", ") + href + " " + width + "w";
    return {
        ...data,
        [reference]: {
            ...(hasReference && prev),
...
export function resizeJobGroupToData(jobGroup) {
    return jobGroup.reduce(reduceResizeJobGroupToData, {});
}
//# sourceMappingURL=resizeJobGroupToData.mjs.map