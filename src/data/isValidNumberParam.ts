 ...
 */
export const isValidNumberParam = (
  n: unknown,
  min: number | string,
  length: number
) => {
  ...
  if (isNaN(minNumber)) {
    console.warn(
      `\`${min}\` can not not be coerced to a number. (minNumberIsNan)`
    );
    return false;
  }
  ...
  if (tooSmall) {
    console.warn(
      `\`${n}\` is too small, expected at least \`${minNumber}\` (tooSmall)`
    );
    return false;
  }
